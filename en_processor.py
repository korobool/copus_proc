...
class CorpusProcessor():
    ...
    def _pos_tag_eng(self, sentence):
        words = list(filter(lambda w: len(w) > 0, re.split('[\s\.\n]', replace_punctuation(sentence))))
        # tags = []
        ...
        tags = map(lambda s: s[1], nltk.pos_tag(words))

        return tuple(tags)

    def _load_sentence(self, line):
        if len(line) <= self.max_length:
            pos_tagged = self._pos_tag_eng(line)
    ...
    def analyze(self, report_path='./'):
        ...
        pr = (lambda template: len(set(template)  # -set(self.punct)
             ) > 4 )# and not 'XXXX' in set(template))
